...
export function createInitialState(featureId) {
    return {
        featureId,
        status: 'in_progress',
        currentStage: 'load',
 ...
 */
export function workflowReducer(state, event, workflow) {
    switch (event.type) {
        case 'BOOTSTRAP': {
        ...
        case 'STAGE_COMPLETE': {
            ...
            if (state.currentStage !== event.stage) {
                throw new Error(`Stage mismatch: expected '${state.currentStage}', got '${event.stage}'`);
            }
        ...
        case 'REOPEN': {
            ...
            const remaining = state.completedStages.filter((s) => workflow.stageOrder.indexOf(s) < targetIndex);
...
export function getNextStage(state, workflow) {
...
export function canExecuteStage(state, stage, workflow) {
    if (state.status === 'blocked') {
