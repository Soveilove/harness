...
export class WorkspaceManager {
     ...
     */
    async syncToSatellite(satelliteId, satelliteStorage) {
        const registry = await this.loadRegistry();
        const satellite = registry.workspaces.find((w) => w.id === satelliteId);
        if (!satellite)
            throw new Error('Satellite not found: ' + satelliteId);
        if (satellite.role === 'hub')
            throw new Error('Target is the hub, nothing to sync');
        const hub = await this.getHub();
        if (!hub)
            throw new Error('No hub workspace registered');
        ...
        for (const type of knowledgeTypes) {
            ...
            if (!hubContent)
                continue;
            const entries = JSON.parse(hubContent);
            ...
            const satEntries = satContent ? JSON.parse(satContent) : [];
            ...
            await satelliteStorage.write('harness/project/knowledge/' + type + '.json', JSON.stringify(merged, null, 2));
     ...
     */
    async promoteToHub(satelliteId, satelliteStorage) {
        const registry = await this.loadRegistry();
        const satellite = registry.workspaces.find((w) => w.id === satelliteId);
        if (!satellite)
            throw new Error('Satellite not found: ' + satelliteId);
        ...
        for (const type of knowledgeTypes) {
            ...
            await this.storage.write('harness/project/knowledge/' + type + '.json', JSON.stringify(hubEntries, null, 2));
        }
        return { promoted };
    }
}
