...
import { getStats, searchEntries } from '../../knowledge/selectors.js';
import { validatePromotion, nextLifecycle } from '../../knowledge/lifecycle.js';
function getStore() {
    return container.inject(TOKENS.KnowledgeStore);
}
...
export function registerKnowledgeCommands(program) {
    ...
    knowledge
        ...
        const target = (opts.to ?? nextLifecycle(entry));
        if (!target) {
        ...
        // Dry-run validation
        const validation = validatePromotion(entry, target, !!(opts.feature && opts.description));
        if (!validation.valid) {
