 ...
 */
export function detectTechStack(packageJson: any, tsconfig: any): DetectedStack {
  ...
  // State management
  if (deps['pinia']) stack.state = 'Pinia';
  else if (deps['vuex']) stack.state = 'Vuex';
  else if (deps['@reduxjs/toolkit'] || deps['redux']) stack.state = 'Redux';
  else if (deps['zustand']) stack.state = 'Zustand';

  // Build tool
  if (deps['vite']) stack.build = stack.build ?? 'Vite';
  else if (deps['webpack']) stack.build = stack.build ?? 'Webpack';
  else if (deps['rollup']) stack.build = stack.build ?? 'Rollup';
 ...
 */
export function generateSeeds(stack: DetectedStack): KnowledgeSeed[] {
 ...
 */
export function seedsToEntries(seeds: KnowledgeSeed[]): Omit<KnowledgeEntry, 'id'>[] {
  const now = new Date().toISOString();
