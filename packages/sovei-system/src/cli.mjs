...
import { buildStageSkillReport, loadSystemConfig, validateSystemConfig } from "./config.mjs";
...
async function main() {
  ...
  if (command === "validate") {
    console.log(JSON.stringify(validateSystemConfig(workflow, skillMap), null, 2));
    return;
  }
