...
> 本文件由 Harness 中枢自动分发，是跨 IDE 通用适配文件。
> Claude Code 读 `CLAUDE.md`，其他工具读本文件（AGENTS.md）。
> 修改请到 `E:\memory\harness\ide-adapters\AGENTS.md`，然后重新分发。
...
2. **不默认运行全量 type-check** — 项目有历史类型错误，全量 `vue-tsc` 输出会掩盖本次改动质量。
