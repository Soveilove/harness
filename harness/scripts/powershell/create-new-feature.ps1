...
if ($Help) {
    ...
    Write-Host "  -NoBranch           Do not create git branch, only create specs directory structure"
    ...
    Write-Host "  ./create-new-feature.ps1 -NoBranch 'Add user authentication'"
    exit 0
}
...
if (-not $DryRun) {
    if ($hasGit -and -not $NoBranch) {
        ...
        Write-Host "[specify] Skipped branch creation (NoBranch mode)"
    } else {
