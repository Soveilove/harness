...
## 公共组件内部状态被多入口绕过 + 外部 watch 破坏性修改共享状态

**问题**: AI 创作自由视角面板中，上传透明图→切黑底→创作记录"一键抠图结果"用作参考图→变白→点切换黑色背景按钮变成"原来的图片"（最初上传图的背景版）。

**根因（两层）**:

### 第一层：route.query watch 路由不匹配，handler 未被调用

`OriginalManuscript` 公共组件内部维护 `originImgSrc`（原始图）和 `curPngBg`（当前背景色）两个状态，其 `route.query watch` 依赖 `route.name === getModuleRouteKeyById(props.module)` 来决定是否触发 `handleImageSelect`。

```
FreePerspectivePanel 传给 OriginalManuscript 的 module = 60（QWEN_FREE_PERSPECTIVE_MODULE_ID）
getModuleRouteKeyById(60) = 'qwenFreePerspective'
但路由 name = 'freePerspective'（对应 module 33）
'freePerspective' !== 'qwenFreePerspective' → handleImageSelect 不被调用
→ originImgSrc 停留图A、curPngBg 停留旧状态
```

### 第二层：外部 watch 时序竞争，破坏性修改共享状态

`FreePerspectivePanel` 有一个 `watch(layoutRefer.image)` 调用 `ensureOpaqueImage` 直接把透明图填白并赋值给 `layoutRefer.image`。这个 watch 先于 `route.query watch` 执行，把原始透明抠图B**破坏性**地变成了不透明白色版。即使后续 `handleImageSelect` 被调用，收到的也是不透明图，`originImgSrc` 丢失原始透明图，切换背景按钮用不透明图填色无效（Canvas 先填色再画不透明图，图覆盖背景）。

```
时序：
1. useInference 赋 layoutRefer = 抠图B
2. FreePerspective watch 先执行 → ensureOpaqueImage(抠图B) → layoutRefer.image = 白色版B（破坏性！）
3. route.query watch 后执行 → handleImageSelect(白色版B) → originImgSrc = 白色版B（不透明）
4. 用户点切换黑色 → addPngBg 用 originImgSrc（不透明图）填黑色 → 图覆盖黑色 → 切换无效
```

**修复（2 处，均在 FreePerspectivePanel/index.vue）**:
1. `OriginalManuscript` 的 `:module` 从 `QWEN_FREE_PERSPECTIVE_MODULE_ID`(60) 改为 `FREE_PERSPECTIVE_STORE_MODULE_ID`(33)，使 route.query watch 匹配，`handleImageSelect` 被调用。
2. 移除 `watch(layoutRefer.image)` 的 `ensureOpaqueImage` 直接改 image 逻辑，避免时序竞争破坏原始透明图。

**修复后透明背景处理收敛为三入口**:
- 上传入口：`OriginalManuscript.handleImageSelect`（保留原始图到 `originImgSrc`，填白后设置 `draftImage`）
- 用作参考图入口：`route.query watch` → `handleImageSelect`（module 传 33 匹配路由）
- 编辑复用等非上传入口：`handleGenerate` 中的 `ensureOpaqueImage` 兜底

**Why**:
- 公共组件（`OriginalManuscript`）内部维护了非受控状态（`originImgSrc`/`curPngBg`），任何修改 `layoutRefer` 的入口若绕过 `handleImageSelect`，都会导致内部状态与外部数据不同步。
- 外部 watch 直接修改组件依赖的共享数据（`layoutRefer.image`）是破坏性的，原始透明图一旦被填白就不可逆，组件内部 `originImgSrc` 无法获取原始透明图，背景切换功能失效。
- 基于 module ID 的路由匹配，传入公共组件的 module ID 必须与路由 name 对应的 module 一致。FreePerspectivePanel 路由 name 是 `'freePerspective'`（module 33），但面板用 module 60 生成，传给 `OriginalManuscript` 时必须用 33。

**How to apply**:
- 公共组件若内部维护非受控状态，所有修改该组件依赖的共享数据的入口，都必须走该组件的 handler（如 `handleImageSelect`），不能在外部直接赋值后指望组件自动同步。
- 外部 watch 不应破坏性修改组件依赖的共享数据（如直接改 image URL 导致原始数据丢失）。透明背景填充等不可逆操作应放在组件 handler 内部（保留原始图到 `originImgSrc`），或在生成提交时兜底（如 `handleGenerate` 中的 `ensureOpaqueImage`）。
- 传给公共组件的 module ID 必须与路由 name 对应的 module 一致，否则基于 `getModuleRouteKeyById` 的路由匹配会失败。`FreePerspectivePanel` 路由 name `'freePerspective'` 对应 module 33，传给 `OriginalManuscript` 必须用 `FREE_PERSPECTIVE_STORE_MODULE_ID`(33) 而非 `QWEN_FREE_PERSPECTIVE_MODULE_ID`(60)。

**调试技巧**: 在关键路径打统一前缀日志（如 `[DEBUG-透明背景]`），覆盖：外部赋值点（`useInference`）、组件路由 watch（`route.query watch`）、组件 handler 入口（`handleImageSelect`）、背景切换入口（`addPngBg`），观察 `originImgSrc`/`curPngBg`/`draftImage` 的值和时序，可快速定位是"handler 未调用"还是"时序竞争"。

**对应项目代码**:
- `src/views/workspace/components/ModulePanel/FreePerspectivePanel/index.vue` — 修复点（module 33 + 移除 watch）
- `src/views/workspace/components/BaseParamComponents/OriginalManuscript.vue` — 公共组件（内部状态 originImgSrc/curPngBg、route.query watch、handleImageSelect、addPngBg）
- `src/views/workspace/hooks/common.ts` — useInference（用作参考图入口，直接赋 layoutRefer）
- `src/views/workspace/board/detail/components/ImageContextToolbar/shared/ensureOpaqueImage.ts` — 透明背景填充工具
