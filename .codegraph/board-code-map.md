...
| 项 | 内容 |
|---|---|
| Code Map 版本 | `2026-07-14.1` |
| 同步范围 | `.codegraph/board-code-map.md` |
| 上级索引 | `.codegraph/recent-work-code-map.md` |
| 维护原则 | 本文件记录自由画布稳定链路；近期地图只保留入口和跳转 |
...
| `helper/hooks/useGenerateParamsBuilder.ts` | 最终参数构建 | `images` 非空时写入 `generateParams.imageList`；MJ 悠船模型走 `imageUrl` 单数字段 + 默认值策略 |
| `hooks/useArtworkGenerationTaskManager.ts` | 任务发起 | clientId、任务状态、错误处理 |
| `hooks/useCanvasTaskListener.ts` | 任务结果回填 | 图片插入、替换、图层同步、自动保存 |

## 悠船（Midjourney）文生图模型（module 66/67/68）

### 模型列表

| 枚举 | ModuleId | 模型键值 | 算力 |
|------|----------|----------|------|
| `MidjourneyV81` | 67 | `v81` | 100 |
| `Midjourney` | 66 | `v7` | 70 |
| `MidjourneyNiji7` | 68 | `niji7` | 70 |

### 生成参数链路

MJ 走 `image-edit` generationType，通过 `AdvancedHeroActions` 底部输入框发起 → `buildGenerateParams` MJ 分支处理。

**关键文件**：
| 文件 | 职责 |
|------|------|
| `list/components/AdvancedHeroActions/midjourneyGenerationConfig.ts` | MJ 模型配置、默认值(`MJ_DEFAULT_VALUES`)、比例选项、参数范围校验 |
| `list/components/AdvancedHeroActions/AdvancedHeroActions.vue` | MJ 参数状态(`mjAspectRatio/Chaos/Stylize/Weird/P/Sref`)、`image-edit` 模式、垫图限制 1 张 |
| `list/components/HeroActions/MidjourneyRatioSelector.vue` | MJ 比例选择器 UI（分辨率固定"自适应"不可选 + 7 个比例预设） |
| `helper/hooks/useGenerateParamsBuilder.ts` | MJ 分支：`imageUrl`（单数垫图）、`chaos/stylize/weird/aspectRatio` 始终带默认值、`--p/--sref` 拼接到 prompt |

### MJ 专属参数映射

| 参数 | 字段名 | 范围 | 默认值 | 适用模型 |
|------|--------|------|--------|----------|
| 垫图 | `imageUrl` | 最多 1 张, ≤20MB | 空 | v8.1/v7/niji7 |
| 比例 | `aspectRatio` | 7 个预设 | `"16:9"` | v8.1/v7/niji7 |
| 多样性 | `chaos` | 0-100 | `5` | v8.1/v7/niji7 |
| 风格化程度 | `stylize` | 0-1000 | `100` | v8.1/v7/niji7 |
| 怪异度 | `weird` | 0-3000 | `100` | 仅 v7/niji7 |
| --p | 拼接到 prompt | - | - | v8.1/v7/niji7 |
| --sref | 拼接到 prompt | - | - | v8.1/v7/niji7 |

**注意**：`imageUrl` 是单数字符串（区别于通用 `imageList` 数组和 Seedream V5 Pro 的 `imageUrls` 复数数组）。`hd` / 分辨率由 API 平台后台固定配置，前端不提交。

### MJ 后台固定参数（不展示，但提交给服务端）

| 参数 | 值 | 适用模型 | 说明 |
|------|-----|----------|------|
| `quality` | `"1"` | v8.1/v7/niji7 | 覆盖通用 quality 为固定值 |
| `iw` | `1` | v8.1/v7/niji7 | Image Weight |
| `sw` | `100` | v8.1/v7/niji7 | Style Weight |
| `sv` | `4` | v8.1/v7/niji7 | Style Version |
| `ow` | `100` | v8.1/v7/niji7 | — |
| `raw` | `false` | v8.1/v7/niji7 | Raw Mode |
| `tile` | `false` | v8.1/v7/niji7 | Tile Mode |
| `hd` | `true` | **仅 v8.1** | Native 2K 开关 |

### 分享详情弹窗

MJ 分享作品的参数展示走依赖包 `@xmiles/holopix-components` 的 `showModalGenerationDetail`。MJ 专属字段（多样性/风格化程度/怪异度/--p/--sref）的展示规则详见：

> **`docs/悠船MJ分享详情弹窗-依赖包字段展示规则.md`**

### 坑点

| 坑点 | 说明 |
|------|------|
| `imageUrl` vs `imageList` | MJ 用单数 `imageUrl` 字段，不用通用数组 `imageList`；`buildGenerateParams` 中 MJ 分支会清空 `imageList` |
| 默认值策略 | 三个模型的 `aspectRatio/chaos/stylize/weird` 始终带默认值提交，不再依赖用户显式设置 |
| `weird` 模型差异 | v8.1 不提交 `weird`，v7/niji7 提交；详情展示亦然 |
| attach 回填 | `_mj*` 字段存入 `attach`，重新编辑/再次生成时从 `attach._mj*` 还原参数 |
| 分享页 module 标签化 | `useShareMediaInteraction` 调用 `getModuleLabelById` 替换 `module` 为中文标签再传给依赖包 |
...
| 改 MJ 悠船模型参数 | `useGenerateParamsBuilder.ts` MJ 分支、`midjourneyGenerationConfig.ts` | `imageUrl` 单数垫图、默认值策略、`weird` 模型差异、`--p/--sref` 拼接 |
| 改 MJ 分享详情展示 | `docs/悠船MJ分享详情弹窗-依赖包字段展示规则.md` → 依赖包 `showModalGenerationDetail` | 需从 `attach._mj*` 解析参数；v8.1 不展示 weird |
