...
| 同步范围 | `.codegraph/recent-work-code-map.md` |
| 使用方式 | 多个独立分支文件夹之间手动复制同步 |
| 同步策略 | 以版本号较新的文件为主；若两个文件都改过，先合并“新增入口/链路”，再提升版本号 |
| 不同步内容 | `.codegraph/*.db`、`cache/`、日志、运行时文件 |
...
> 自由画布已拆分为专用地图：`.codegraph/board-code-map.md`。本节只保留近期总览入口，复杂链路、参数映射、局部修改和一键成稿细节统一维护在专用地图中。
...
| `.codegraph/board-code-map.md` | 自由画布专用地图 | 复杂链路、参数流、面板细节、常见任务定位 |
...
| 改 MJ 悠船生图参数/分享 | `.codegraph/board-code-map.md#悠船midjourney文生图模型module-666768`、`docs/悠船MJ分享详情弹窗-依赖包字段展示规则.md` | 参数映射看 code map，分享弹窗展示规则看 docs |
...
- 多个独立工作目录之间同步 `.codegraph/recent-work-code-map.md` 和 `.codegraph/board-code-map.md`，不复制 `.codegraph` 数据库、缓存、日志和运行时文件。
...
- 自由画布复杂链路统一维护在 `.codegraph/board-code-map.md`，近期地图只保留入口索引。
