...
**依据**：`design-docs/产品功能需求.md`、`doc/开发归档.md`、`design-docs/工作流设计.md`
...
`doc/开发归档.md` 记录的是历史完成项和旧设计差距，不能覆盖当前产品需求。后续产品判断以源码、产品验证结果和 `design-docs/产品功能需求.md` 为准，历史文档只用于理解演进原因。
...
历史工程排序曾将 R1.2 排在 R1.3 之前，主要是状态模型依赖和历史 split 协议修复的工程安排。当前产品排期应先验证 R1.3 的单 Feature 路由闭环；在 R1.2 完成前，`split-candidate` 只允许进入正式 `grill`，不能执行真正的 split。这样既验证最重要的用户价值，也不把未稳定的 split 机制带入新需求。
