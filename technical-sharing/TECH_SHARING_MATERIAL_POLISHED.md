...
- `load`：盘点出所有 Node 14 不兼容点（发布产物是 ESM、`import.meta.url`/`import.meta.dirname` 不可用、`.at()` 是 Node 16.6+ 的 API、测试框架 `node:test` 需 Node 18）。
- `grill`：确认目标范围——**只有发布产物需要兼容 Node 14**，开发/构建/测试链路仍用 Node 20+。这个边界把工作量从"全链路降级"缩到"改产物形态 + 替换几个 API"。
- `wayfind`：S1 小型工作，走 `skip`，不生成决策地图。
- `spec`/`scope`/`plan`/`tasks`：把方案收敛为 6 个纵向任务，产出 `reconciliation.md`、`coverage-matrix.md`、`plan.md`。
- `implement`：实际改动，发布产物 ESM → CommonJS，`engines.node` 从 `>=20` 放宽到 `>=14.18.0`。
- `converge`：审查发现一个**高严重度问题**——改产物格式后，基于产物文本的"未内联外部依赖"校验把 Node 内置模块和第三方库的字符串文案误判为外部依赖，导致构建必然失败。已修复并用 `isBuiltin` 排除内置模块、收紧正则。
- `verify`：`tsc --noEmit`、107/107 测试、`verify:package` 全绿；再**手动下载 Node 14.21.3** 实测 `--version`/`--help`/`project status`/`workflow status` 全部正常。
- `learn`/`sync`：3 条观察回流知识库（产物形态匹配目标环境、跨版本兼容要精确到子版本、产物格式变更要重审文本校验）。

完整的决策、方案、证据和知识都沉淀在 `specs/017-node14-compat/` 下，任何一个阶段都可以跨会话恢复、可以审计、可以回放。

### 关于成本：1600 万 token 是缺点吗

这次 Feature 完整走下来，一次性消耗约 1600 万 token。如果只看"我改了几行代码"，这个数字看起来高得吓人。但它买来的不是这几行代码，而是：

- **可审计的决策链**：为什么产物要改 CJS、为什么不改源码里的 `import.meta`、最低子版本为什么是 14.18——每个问题都有记录。
- **真实的证据**：Node 14 不是"理论上兼容"，而是用真机实测通过的 4 个命令路径。
- **自动沉淀的知识**：`converge` 阶段差点放过一个会让构建必然失败的 bug，正是"审查"这个环节把它拦住；3 条经验回流知识库，下次遇到同类问题不必重新踩坑。
- **可回放的过程**：12 个阶段每一个都有独立产物，过程中断也能从任意阶段恢复。

更深一层，这恰好回答了一个问题：**为什么便宜模型有可能逼近顶级模型的效果？** 因为 Sovei 的每个阶段都是"带契约的执行"，而不是"自由发挥"。模型不需要靠自己的推理去覆盖整个系统的复杂度，它只需要：

```text
读取阶段契约 + 上下文包 → 按契约产出该阶段的产物 → 通过校验进入下一阶段
```

模型的天赋被用在"执行清晰的契约"上，系统的复杂度由阶段、红线、决策记录和证据承担。1600 万 token 买的是**确定性**——顶级模型"一次生成"给不了的：过程可审计、结果可验证、经验可复用。

### 成本收益的取舍

这不是说 1600 万 token 无所谓，而是说它要放在正确的对比维度上：

| 维度 | 直接生成代码 | Sovei 工作流 |
|---|---|---|
| 产出 | 几行/几十行代码 | 完整决策链 + 证据 + 知识回流 |
| 可审计性 | 低 | 高（12 阶段产物全在 specs/） |
| 可恢复性 | 无 | 高（任意阶段可续跑） |
| 经验复用 | 无 | 高（learn 自动回流知识库） |
| 防回归 | 靠 review 纪律 | 收敛审查 + 真机验证 + 知识沉淀 |

所以我的结论是：**高 token 不是缺陷，它是"把不确定性转化为可治理资产"的代价**。如果只追求"便宜地把活干完"，用便宜模型直接生成即可；但若要"这套流程能自我迭代、跨会话累积、每次变更都可回放"，这些 token 是必要的投入。这也是我用便宜模型驱动完整 Sovei 流程、仍然想追求顶尖模型效果的初衷——用流程的确定性，去补模型个体能力的差距。
